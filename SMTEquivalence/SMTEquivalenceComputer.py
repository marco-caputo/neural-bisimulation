...
from SMTEquivalence import are_approximate_equivalent
...
def compute_approximate_equivalence_2(model1: NeuralNetwork | torch.nn.Module | tf.keras.Model,
    ...
    # Prepare the solver
    s = Solver()
    s.add(And([inputs1[i] == inputs2[i] for i in range(len(inputs1))]))  # Input equality
    s.add(formula1)
    s.add(formula2)
    ...
    for i in range(math.ceil(math.log(1 / precision, 2))):
        s.push()
        s.add(are_not_approximate_equivalent_formula(p, epsilon)(outputs1, outputs2))

        result = s.check()
        if result == sat:  # Not (p,epsilon)-approximate equivalent
            lower = epsilon
        else:
            upper = epsilon

        if verbose:
            print(("Found counterexample" if result == sat else (
                    "No counterexample" if result == unsat else
                    "Unknown result")) + " for epsilon = " + str(epsilon))
